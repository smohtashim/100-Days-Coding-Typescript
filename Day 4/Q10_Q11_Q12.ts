...
let my_name : string = "Syed Mohtashim Ali"; // Declare string variable my_name and stores the value in it.
...
function TitleCase(str: string): string { // there is no built-in function for titlecase, so we need to create it own.
...
console.log("Titlecase:", TitleCase(my_name));
 ...
 // Q11
 let names: string[] = ["Zain","Zeeshan","Shahzaib","Owais"];
 for (let i = 0; i  < names.length; i++) {
    console.log(names[i]);
 } 

 // Q12
 let f_names: string[] = ["Shahzaib","Zain","Owais","Zeeshan"];
 f_names.forEach (f_name => {
    console.log(`Hello ${f_name}! How are you?`);
 });
